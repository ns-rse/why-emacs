...
* [pass](https://www.passwordstore.org/) uses GPG encryption to store passwords.
* Access passwords with a single key-binding.
* Linked to Yubikey, prompted for password (if not already unlocked).
* Easy to encrypt text files to GPG, just save with `.gpg`.
* Opening `.gpg` files simple if GPG key unlocked with Yubikey.
...
one. It gives you a highly customised environment that works they way you want it to, but wait for the con, and there is
